...
currency_summary_stats <- fx_processed |>
  ...
  dplyr::arrange(market_group, from)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# PRE-ANALYSIS 9a: Fama-MacBeth Regressions
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# PRE-ANALYSIS 9b: Fama-MacBeth Newey-West Robustness
# ------------------------------------------------------------------------------

# Computes Fama-MacBeth statistics for alternative Newey-West lag lengths
fmb_nw_stats_lag <- function(df, nw_lag) {
  df <- df |>
    dplyr::arrange(date) |>
...
# Alternative Newey-West lag specifications
nw_lags <- c(0, 1, 3, 6, 12)
fmb_nw_robustness <- purrr::map_dfr(
  nw_lags,
  function(L) {
       carry <- fmb_carry_step1 |>
      dplyr::group_by(market_group) |>
      dplyr::group_modify(
        ~ fmb_nw_stats_lag(.x, nw_lag = L)) |>
      dplyr::ungroup() |>
      dplyr::mutate(
        strategy = "CS-Carry",
        nw_lag = L)
    momentum <- fmb_mom1_step1 |>
      dplyr::group_by(market_group) |>
      dplyr::group_modify(
        ~ fmb_nw_stats_lag(.x, nw_lag = L)) |>
      dplyr::ungroup() |>
      dplyr::mutate(
        strategy = "MOM-1M",
        nw_lag = L)
    dplyr::bind_rows(carry, momentum)
  }
)

# Andrews (1991) automatic Bartlett bandwidth
andrews_bandwidth <- function(lambda_t) {
  
  lambda_t <- stats::na.omit(lambda_t)
  T <- length(lambda_t)
  
  ar1_model <- stats::lm(
    lambda_t[-1] ~ lambda_t[-T]
  )
  
  rho <- unname(stats::coef(ar1_model)[2])
  
  alpha1 <- (4 * rho^2) /
    ((1 - rho)^2 * (1 + rho)^2)
  
  bandwidth <- 1.1447 * (alpha1 * T)^(1/3)
  
  tibble::tibble(
    T = T,
    rho = rho,
    bandwidth = bandwidth,
    implied_lag = floor(bandwidth)
  )
}

andrews_carry <- fmb_carry_step1 |>
  dplyr::arrange(date) |>
  dplyr::group_by(market_group) |>
  dplyr::group_modify(
    ~ andrews_bandwidth(.x$lambda_t)) |>
  dplyr::ungroup() |>
  dplyr::mutate(strategy = "CS-Carry")

andrews_mom1 <- fmb_mom1_step1 |>
  dplyr::arrange(date) |>
  dplyr::group_by(market_group) |>
  dplyr::group_modify(
    ~ andrews_bandwidth(.x$lambda_t)) |>
...
thesis_data <- list(
  ...
  fmb_nw_robustness = fmb_nw_robustness,
  fmb_andrews = fmb_andrews
)

saveRDS(thesis_data, "thesis_data.rds")
print("All analyses processed flawlessly! Data saved as thesis_data.rds")